var simpleExample = `acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf`;
var testInput =
...
function sortWords(myArray) {
  for (let index = 0; index < myArray.length; index++) {
    myArray[index] = myArray[index].split('').sort().join('');
  }
  return myArray;
}

function getDigitForSegments(segments) {
  var singleString = segments.join('');
  switch (singleString) {
    case 'abcefg':
      return 0;
    case 'cf':
...
var cryptedA, cryptedB, cryptedC, cryptedD, cryptedE, cryptedF, cryptedG;

function processLine(line) {

  var prefix = line.split(` | `)[0];
  var words = prefix.split(' ');
  twoDigitWords = sortWords(words.filter(word => word.length == 2));
  threeDigitWords = sortWords(words.filter(word => word.length == 3));
  fourDigitWords = sortWords(words.filter(word => word.length == 4));
  fiveDigitWords = sortWords(words.filter(word => word.length == 5));
  sixDigitWords = sortWords(words.filter(word => word.length == 6));
  sevenDigitWords = sortWords(words.filter(word => word.length == 7));
  // console.log('2 digts: ', twoDigitWords);
  // console.log('3 digts: ', threeDigitWords);
  // console.log('4 digts: ', fourDigitWords);
  // console.log('5 digts: ', fiveDigitWords);
  // console.log('6 digts: ', sixDigitWords);
  // console.log('7 digts: ', sevenDigitWords);

  cryptedA = determineA();
  cryptedD = determineD(cryptedA);
  cryptedG = determineG(cryptedA);
  var knownChars = [cryptedA, cryptedD, cryptedG];
  knownChars.push(...twoDigitWords[0].split(''));
  cryptedB = determineB(knownChars)
  knownChars = [cryptedA, cryptedB, cryptedD, cryptedG];
  cryptedF = determineF(knownChars);
  knownChars = [cryptedF];
  cryptedC = determineC(knownChars);
  knownChars = [cryptedA, cryptedB, cryptedC, cryptedD, cryptedF, cryptedG];
  cryptedE = determineE(knownChars);

  // console.log('a wird angezeigt als ', cryptedA);
  // console.log('d wird angezeigt als ', cryptedD);
  // console.log('g wird angezeigt als ', cryptedG);
  // console.log('b wird angezeigt als ', cryptedB);
  // console.log('f wird angezeigt als ', cryptedF);
  // console.log('c wird angezeigt als ', cryptedC);
  // console.log('e wird angezeigt als ', cryptedE);

  var suffix = line.split(` | `)[1];
  var outputValue = suffix.split(' ');
  var result = 0;
  for (let index = 0; index < 4; index++) {
    var tmp = decryptValue(outputValue[index]);
    result += getDigitForSegments(tmp.sort());
    if (index < 3) {
      result *= 10;
    }
  }
  return result;
  // return (outputValue.filter(word => (word.length != 5 && word.length != 6))).length;
}
...
function getCommonCharsOfFive() {
  var fiveLetters1 = fiveDigitWords[0].split('');
  var fiveLetters2 = fiveDigitWords[1].split('');
  var fiveLetters3 = fiveDigitWords[2].split('');
  var filteredArray = fiveLetters1.filter(value => fiveLetters2.includes(value));
  filteredArray = filteredArray.filter(value => fiveLetters3.includes(value));
  // console.log(`Common chars in 5 digit words: `, filteredArray);
  return filteredArray;
}

function determineA() {
  var twoLetters = twoDigitWords[0].split('');
  var threeLetters = threeDigitWords[0].split('');
  for (let index = 0; index < 2; index++) {
    if (twoLetters[index] < threeLetters[index]) {
      return twoLetters[index];
    } else if (twoLetters[index] > threeLetters[index]) {
      return threeLetters[index];
    } else {
      // characters were the same, check next one
    }
  }
  return threeLetters[2];
}
function determineB(knownChars) {
  var fourLetters = fourDigitWords[0].split('');
  return fourLetters.filter(value => knownChars.includes(value) == false)[0];
}
function determineC(knownChars) {
  var twoLetters = twoDigitWords[0].split('');
  return twoLetters.filter(value => knownChars.includes(value) == false)[0];
}
function determineD(cryptedA) {
  // find the three characters common to all five-digit words (this will include 'a')
  var filteredArray = getCommonCharsOfFive();

  var fourLetters = fourDigitWords[0].split('');

  // remove already solved 'a'
  filteredArray = filteredArray.filter(value => value != cryptedA);
  // console.log(`Common chars in 5 digit words (without already known letter "`, cryptedA, `" for "a"): `, filteredArray);

  filteredArray = filteredArray.filter(value => (fourLetters.includes(value) == true));
  // var twoLetters = twoDigitWords[0].split('');
  // filteredArray = filteredArray.filter(value => (twoLetters.includes(value) == false));
  return filteredArray[0];
  // intersect with the characters in the four digit word, that is neither a nor within the two digit word
  var remainingThreeLetters = fourDigitWords[0].replace(cryptedA, '').split('').sort();
}

function determineE(knownChars) {
  return ['a', 'b', 'c', 'd', 'e', 'f', 'g'].filter(value => (knownChars.includes(value) != true))[0];
}

function determineF(knownChars) {
  for (let index = 0; index < fiveDigitWords.length; index++) {
    var fiveLetters = fiveDigitWords[index].split('');
    var filteredArray = fiveLetters.filter(value => (knownChars.includes(value) != true));
    if (filteredArray.length == 1) {
      return filteredArray[0];
    }
  }
}
function determineG(cryptedA) {
  // find the three characters common to all five-digit words (this will include 'a')
  var filteredArray = getCommonCharsOfFive();
...
function solve(digits) {
  ...
  for (let index = 0; index < lines.length; index++) {
    foundDigits += processLine(lines[index]);
  }
  return foundDigits;
}
...
describe('testMySolution', function () {
  it('Beispiele', function () {
    assert.equal(solve(simpleExample), 5353);
    assert.equal(solve(testInput), 61229);
