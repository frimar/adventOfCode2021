...
function getTargetPosition(allCrabs) {
    return getMedian(allCrabs);
}


function getFuelCost(fuel, crabs, index, target) {
    var n = Math.abs(crabs[index] - target);
    return (n * (n + 1) / 2);
}


function solve(crabs, targetPosition) {

    let target = getTargetPosition(crabs);
    ...
    for (let index = 0; index < crabs.length; index++) {
        fuel += getFuelCost(fuel, crabs, index, target);
    }
    return fuel;
}
...
describe('testMySolution', function () {
    it('Beispiele', function () {
        assert.equal(solve(testInput, 5), 168);
        assert.equal(solve(testInput, 2), 206);

        var median = getMedian(realInput);
        var direction = 'check';
        var done = false;
        var fuel_median = solve(realInput, median);
        var fuel_up = solve(realInput, median + 1);
        var fuel_down = solve(realInput, median - 1);

        if (fuel_down < fuel_median) {
            direction = 'down';
        } else if (fuel_up < fuel_median) {
            direction = 'up';
        } else {
            console.log('check 2 steps ahead....');
            done = true;
        }
        while (!done) {
            switch (direction) {
                case 'up':
                    median++;
                    var tmp = solve(realInput, median);
                    if (tmp > fuel_median) {
                        // fuel rising again => solved!
                        done = true;
                        console.log('Best position: ', median - 1);
                        console.log('Fuel: ', fuel_median);
                    } else {
                        fuel_median = tmp;
                    }
                    break;
